...
public class JPACriteriaEmployeeFinder implements EmployeeFinder {
    ...
    private Predicate addTitlesPredicate(EmployeeSearchCriteria criteria, CriteriaBuilder cb, Root employee, Predicate predicate) {
        if (criteria.getTitles() != null && criteria.getTitles().size() > 0) {
            Join titlesJoin = employee.join(Employee_.titles);
            predicate = cb.and(predicate, titlesJoin.get("id").get("title").in(criteria.getTitles()));
            predicate = cb.and(predicate,cb.equal(titlesJoin.get("toDate"), TimeProvider.MAX_DATE));
        }
        return predicate;
    }
    ...
    private Predicate addSalaryToPredicate(EmployeeSearchCriteria criteria, CriteriaBuilder cb, Root employee, Predicate predicate)  {
        if(criteria.getSalaryFrom() != null) {
            Join salaryJoin = employee.join(Employee_.salaries);
            predicate = cb.and(predicate, cb.lessThanOrEqualTo(salaryJoin.get("salary"), criteria.getSalaryTo()));
            predicate = cb.and(predicate, cb.equal(salaryJoin.get("toDate"), TimeProvider.MAX_DATE));
        }
        return predicate;
    }
    ...
    private Predicate addSalaryFromPredicate(EmployeeSearchCriteria criteria, CriteriaBuilder cb, Root employee, Predicate predicate) {
        if(criteria.getSalaryFrom() != null) {
            Join salaryJoin = employee.join(Employee_.salaries);
            predicate = cb.and(predicate, cb.greaterThanOrEqualTo(salaryJoin.get(Salary_.salary), criteria.getSalaryFrom()));
            predicate = cb.and(predicate, cb.equal(salaryJoin.get("toDate"), TimeProvider.MAX_DATE));
        }
        return predicate;
    }
